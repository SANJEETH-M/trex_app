...
function draw() {
  ...
  if(ground.x<0) {
    ...
    if(trex.y<height-33.5){
      trex.changeAnimation("trex_jump");
    }
    
  //to make the trex jump by space
   if((touches.length>0 || keyDown("space"))&&trex.y>=height-32.5) {
     trex.velocityY=-10;
     jumpSound.play();
     touches=[];
   } 
  ...
  if((touches.length>0 || keyDown("up"))&&trex.y>=height-32.5) {
     trex.velocityY=-10;
     jumpSound.play();
    touches=[];
   } 
...
function spawnClouds() {
  if(frameCount%100==0)  {
  cloud=createSprite(width,height-100,10,10);
  cloud.velocityX=-2;
    cloud.addImage("cloud",cloudImage);
    cloud.scale=0.5;
    cloud.y=Math.round(random(height-200,height-50));
